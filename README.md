...
├── tasks/<task-id>/        # State layer: spec / status / validation / decisions — the recovery point of every run
└── docs/                   # Design docs, goal contracts, architecture diagrams
```

Scenarios and the registry graph are accelerators by design, not authority: execution order comes from scenario `order` or the task-declared topology, graph edges gate nothing, and neither layer being thin weakens the gates. The durable assets of this architecture are the dual-mode gated pipeline, the planning/implementation context split, and the task files as session-external memory.

### Execution Pipeline

Both entry modes converge on one gated pipeline. Planning happens in a single session so cross-repo judgments stay coherent (ingestion may be delegated to read-only subagents; synthesis may not). Implementation is delegated per repository to child agent processes started inside each repository, so repo-local runtime mechanisms (hooks, skills, MCP injection) activate normally.

```mermaid
flowchart TB
    S["Scenario task<br/>(topology pre-declared in scenario.yaml)"]
    F["Free task<br/>(topology generated by planning, approved by Spec Review)"]
    S -->|"validate-scenario"| INIT
    F -->|"validate-registry"| INIT
    INIT["init-task [--free] → four task files"] --> PF["preflight: branch gates + git state"]
    PF --> PL["Planning Pass — single session:<br/>cross-repo spec + repo-local spec entries"]
    PL <-->|"planning_blocked → user"| U(("user"))
    PL --> G1{{"Planning Gate"}}
    G1 --> SR["Spec Review<br/>(human approval of topology + spec-entry diffs)"]
    SR -->|"approved / explicit skip"| G2{{"Spec Review Gate"}}
    G2 --> RUN
    subgraph run["run: deterministic per-repo chain, declared order"]
        R["child agent in repo dir:<br/>runtime reconciliation → implement"] --> VD{"verdict ok?"}
        VD -->|"missing / malformed / blocked"| STOP(["run blocked"])
        VD -->|"ok"| CK["repo checks"]
        CK -->|"fail"| STOP
        CK -->|"pass"| RC["repo_complete → next repo"]
    end
    RC --> DONE(["complete: all gates recorded, all repos green"])
```

Fail-closed behavior is mode-independent:

- `run` refuses to start without both gates recorded in `status.md` (exit 2, `planning_gate_missing` / `spec_review_gate_missing`).
- A repository counts as complete only with exit 0, a valid `ok` verdict (`tasks/<task>/verdicts/<repo>.md`), and passing checks.
- Discovering a new affected repository mid-run goes through the Replanning Protocol and a fresh Spec Review — never a silent expansion.

Static diagrams live in `docs/diagrams/` (`freeform-workspace.png`, `run-architecture.png`); the `run` layer is documented in [`docs/subprocess-agent-run.md`](docs/subprocess-agent-run.md).
